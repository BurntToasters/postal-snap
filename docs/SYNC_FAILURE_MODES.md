...
| W10 | A pooled session outlives an account removal or a password change and keeps using old credentials. | The pool entry is dropped on removal and on `update_account_password`. | GreenMail `greenmail_protocol_integration_history` (after `forget`, a wrong password is really sent) |
