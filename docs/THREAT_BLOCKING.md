...
- HTML and plain-text links share one inspect / confirm / open path. Opening goes through Rust. The frontend opener plugin is not a policy bypass. The Apple app-password help page is a fixed URL that still passes through native `inspect_external_link` and `authorize_external_open(..., false)`, so it cannot skip the reported-threat gate.
