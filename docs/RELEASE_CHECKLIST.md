...
- [ ] Windows x64 and arm64 NSIS installers have valid Azure Artifact Signing Authenticode signatures and updater signatures. Prove `Get-AuthenticodeSignature` on the installed `Postal Snap.exe`, not only setup.exe. Run `npm run setup:win:artifact-signing` once as Administrator on the VM before `release:win`.
